    ...
    <v-layout>
        <v-flex>
          <v-stepper v-model="current_index" non-linear>
            <v-stepper-header>
              <v-stepper-step editable :color=color(1) complete-icon="mdi-check" edit-icon="mdi-check" @click=move(1) :complete=visited(1) step="1">Tabulky</v-stepper-step>

              <v-divider></v-divider>

              <v-stepper-step editable :color=color(2) complete-icon="mdi-check" edit-icon="mdi-check" @click=move(2) :complete=visited(2) step="2">Metadata</v-stepper-step>

              <v-divider></v-divider>

              <v-stepper-step editable :color=color(3) complete-icon="mdi-check" edit-icon="mdi-check" @click=move(3) :complete=visited(3) step="3">Procedury</v-stepper-step>
            </v-stepper-header>
            ...
            <v-stepper-items>
              <v-stepper-content step="1">
                <v-card class="mb-12" flat>
                  <v-card-text>
                    <img src="../assets/relations/relations.png">
                    ...
                    <p>
                      ...
                      Ta má na na každém řádku sloupečky o autě (např.: značka, barva, typ, typ karoserie, datum výroby, kvartál výroby, rok výroby, platnost STK ...), sloupečky o majiteli (firma, ičo, jméno, příjmení, číslo technického průkazu, obec, kraj, zemi, datum a rok jeho vydání, 
                      zákaznický segment majitele, počet vozidel, která už koupil, částku, kterou u nás doposud utratil, částku utracenou minulý rok atd.), pak třeba ještě informace o prodeji (cenu, prodejnu, obchodníka, dodací lhůtu a tak). Prostě všechno na jedné hromadě, už pospojované 
                      a často se zdánlivě zbytečnými sloupci, které ale usnadňují analýzu.
                    </p>
                  </v-card-text>
                </v-card>
